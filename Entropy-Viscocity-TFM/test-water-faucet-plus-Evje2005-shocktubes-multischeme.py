        ...
        while t < self.t_final:
            ...
            for i in range(1, self.N):
                ...
                if 'Upwind1' in scheme:
                    ...
                    # Semi-discrete central-upwind (So, 2016; Section 6.1) with anti-diffusion.
                    # Implemented in a vector form for U = [mass, mom].
                    ...
                    U_L = np.array([mass_L, mom_L])
                    U_R = np.array([mass_R, mom_R])
                    F_L = np.array([mom_L, mom_L * u_L_face])
                    F_R = np.array([mom_R, mom_R * u_R_face])
                    ...
                    if denom < 1e-14:
                        ...
                        H = 0.5 * (F_L + F_R) - 0.5 * s * (U_R - U_L)
                    else:
                        # Intermediate state U* (Eq. 49) and anti-diffusion d (Eq. 48)
                        U_star = (a_plus * U_R - a_minus * U_L - (F_R - F_L)) / denom
                        F_star = np.array([
                            U_star[1],
                            (U_star[1]**2) / max(U_star[0], 1e-14),
                        ])
                        d = minmod(F_R - F_star, F_star - F_L)

                        H = (a_plus * F_L - a_minus * F_R + a_plus * a_minus * (U_R - U_L)) / denom - 0.5 * d

                    f_mass_conv = float(H[0])
                    f_mom_conv  = float(H[1])
                ...
                # Convective Flux (non-CU schemes only)
    ...
    if args.problem == "faucet":
        ...
        CASES = {
            ...
            "TOUMI": {
                ...
                "N": 500, "L": 100.0, "t_final": 0.08, "dxdt": 1000.0, "sigma": 1.0,
            },
        }
        ...
        def run_one(sch: str):
            ...
            print(f"[ShockTube {args.problem}] running scheme={sch} | dt_mode={args.dt_mode} | "
